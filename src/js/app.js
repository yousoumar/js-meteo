...
locationButton.addEventListener("click", () => {
  ...
  if (!(lat && long)) {
    infoHandle(
      `<p>Veuillez autoriser la géolocalisation lorsque votre navigateur vous la demande.<p>`
    );
    setTimeout(() => {
      ...
      if (navigator.geolocation) {
        ...
        loader.style.display = "none";
        infoHandle(
          `<p>Votre navigateur ne peut faire de la géolocalisation. Chercher votre ville par son nom.</p>`
        );
      }
    }, 3000);
  } else {
...
function callApiLatLong(lat, long) {
  fetch(
    ...
    .then((data) => {
      ...
      let ordredDays = days
        .slice(days.indexOf(localDay))
        .concat(days.slice(0, days.indexOf(localDay)));
      ...
      for (let m = 0; m < weekend.length; m++) {
        weekend[m].innerHTML = `<div>${ordredDays[m]}</div>
                                       ...
                                       )}°C</span> <span class="min">${Math.trunc(
          data.daily[m].temp.min
        )}°C</span></div>`;
      }
    ...
    .catch(() => {
      ...
      infoHandle(
        `<p>Oups, il y a un petit soucis avec notre API, revenez plus tard svp. :)<p>`,
        true
      );
    });
}
